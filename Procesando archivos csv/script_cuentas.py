...
def aplicar_transferencias( cuentas, nro_de_cuenta_destino ,transferencias):
    for cuenta in cuentas:
        if cuenta[0] == nro_de_cuenta_destino:
            cuenta[2] = str(float(cuenta[2]) + transferencias) 
            break
    return cuentas       


def procesar_transferencias(cuentas):
    archivo = open("transferencia.csv") 
    primera_linea = True
    for linea in archivo:
        if not primera_linea:
            nro_de_cuenta_destino, monto_transferencias = linea.replace('\r\\n', '').split(',')
            importe_transferencias_float = float(monto_transferencias)
            cuentas_actualizadas =aplicar_transferencias (cuentas, nro_de_cuenta_destino , importe_transferencias_float)
        else:
            primera_linea = False
    archivo.close()
    return cuentas_actualizadas 
...
def main():
    ...
    print("aplicando transferencias . . .")
    cuentas_actualizadas_ = procesar_transferencias(cuentas)
    imprimir_cuentas(cuentas_actualizadas)
